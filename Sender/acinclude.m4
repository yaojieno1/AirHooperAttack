...
yyydebug=yes, yyydebug=no)

AC_ARG_ENABLE(profiling,
[  --enable-profiling      every program creates profiling output],
yyyprofiling=yes, yyyprofiling=no)

AC_ARG_ENABLE(nooptimizing,
[  --enable-nooptimizing   do not optimize],
yyynooptimizing=yes, yyynooptimizing=no)

AC_ARG_ENABLE(nowarnerror,
[  --enable-nowarnerror    do not stop make if compiler warning],
yyynowarnerror=yes, yyynowarnerror=no)


AC_ARG_WITH(cccxxpre,
[  --with-cccxxpre=x       add at beginning of C and C++ options],
yyycccxxpre="$withval",yyycccxxpre="")

AC_ARG_WITH(cccxxpost,
[  --with-cccxxpost=x      add at end of C and C++ options],
yyycccxxpost="$withval",yyycccxxpost="")


AC_ARG_WITH(ldpre,
[  --with-ldpre=x          add at beginning of linker options],
yyyldpre="$withval",yyyldpre="")

AC_ARG_WITH(ldpost,
[  --with-ldpost=x         add at end of linker options],
yyyldpost="$withval",yyyldpost="")
...
if test "$yyynooptimizing" = "yes"; then
  YYY_CFLAGS="$YYY_CFLAGS -O0"
else
  YYY_CFLAGS="$YYY_CFLAGS -O2"
fi

if test "$yyyprofiling" = "yes"; then
  yyydebug=yes
  YYY_CFLAGS="$YYY_CFLAGS -pg"
  YYY_LDFLAGS="$YYY_LDFLAGS -pg"
fi
...
if test "$yyynowarnerror" = "yes"; then
  YYY_CFLAGS="$YYY_CFLAGS -Wall"
else
  YYY_CFLAGS="$YYY_CFLAGS -Wall -Werror"
fi
...
LDFLAGS="$yyyldpre $YYY_LDFLAGS $yyyldpost"
